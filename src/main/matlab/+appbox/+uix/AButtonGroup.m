classdef AButtonGroup < handle
    ...
    methods
        ...
        function ca1ButtonLabels = get.Buttons(obj)
            ...
            ca1ButtonLabels = arrayfun(@(b)b.String, obj.g1GroupHandles, 'UniformOutput', false);
        end
        ...
        function set.Enable(obj,sToggle)
            ...
            assert( isa( sToggle, 'char' ) || iscellstr( sToggle ), 'uix2:InvalidPropertyValue', ...
                'Property ''Enable'' must be a string or cell array of strings.' )
            if iscellstr(sToggle)
                assert( numel(sToggle) == numel(obj.g1GroupHandles), 'uix2:InvalidPropertyValue', ...
                    'Property ''Enable'' have the same number of elements as Buttons' )
            end
            
            if ischar(sToggle)
                set( findall(obj, '-property', 'Enable'), 'Enable', sToggle);
            else
                for i = 1:numel(sToggle)
                    obj.g1GroupHandles(i).Enable = sToggle{i};
                end
            end
        ...
        function onButtonPress(obj,source,~)
            ...
            if inList
                ...
                if ~isempty(obj.SelectionChangeFcn)
                    feval(obj.SelectionChangeFcn, obj, eventdata);
                end
